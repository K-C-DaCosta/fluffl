    ...
    T: Default + Copy + HasConstants,
{
    ...
    pub fn from_array<M>(data: [M; N]) -> Self
    where
        M: Copy + Into<T> + Default,
    {
        let mut converted_data = [T::default(); N];
        converted_data
            .iter_mut()
            .zip(data.iter())
            .for_each(|(converted, &input)| *converted = input.into());
...
where
    T: Add<Output = T> + HasConstants + Default + Copy,
{
    fn sum<I: Iterator<Item = Self>>(iter: I) -> Self {
        iter.fold(Vector::zero(), |acc, v| acc + v)
    }
}
