...
impl FlufflWindow {
    ...
    pub fn main_loop<Loop, State>(
        mut fluffl_window: Self,
        app_state: State,
        core_loop: impl Fn(FlufflWindowPtr, Rc<Cell<bool>>, Rc<RefCell<State>>) -> Loop + Copy + 'static,
    ) where
        Loop: std::future::Future<Output = ()>,
        State: 'static,
    {
        ...
        render_loop.run(move |running| {
            ...
            let f = core_loop(window_ptr.clone(), local_run.clone(), aux_data_ptr.clone());
            futures::executor::block_on(f);
            *running = local_run.get()
        });
    }
}
...
fn map_scancode(scancode: sdl2::keyboard::Scancode) -> KeyCode {
    match scancode {
        ...
        Scancode::LeftBracket => KeyCode::BRACKET_LEFT,
        Scancode::RightBracket => KeyCode::BRAKET_RIGHT,
        ...
        Scancode::Slash => KeyCode::FORWARD_SLASH,
        ...
        Scancode::PageDown => KeyCode::PAGE_DOWN,
        Scancode::PageUp => KeyCode::PAGE_UP,
        Scancode::Left => KeyCode::ARROW_LEFT,
        Scancode::Right => KeyCode::ARROW_RIGHT,
        Scancode::Up => KeyCode::ARROW_UP,
        Scancode::Down => KeyCode::ARROW_DOWN,
