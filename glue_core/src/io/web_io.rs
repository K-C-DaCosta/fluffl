...
async fn load_file_helper(file_path: &str) -> Result<Vec<u8>, GlueError> {
...
impl From<JsValue> for GlueError {
    fn from(js_error: JsValue) -> Self {
        GlueError::IOError(String::from(""))
    }
}
