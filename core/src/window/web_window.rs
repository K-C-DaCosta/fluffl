...
impl FlufflWindow {
    ...
    /// ```
    pub fn main_loop<Loop, LoopRet, State>(mut fluffr_window: Self, aux: State, core_loop: Loop)
    where
        State: 'static,
        ...
        render_loop.run(move |running| {
            if window_ptr.window_mut_cb(|win| transfer_events(win)) {
                ...
                spawn_local(core_loop(
                    win_ptr_clone,
                    FlufflRunning::with_rc_cell(local_running.clone()),
                    FlufflState::new(aux_ptr_clone),
                ));
