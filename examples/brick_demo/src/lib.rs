...
type ShortDeviceCore = AudioDeviceCore<ShortCallback, ShortState>;
...
pub fn draw_game_stage(
    ...
    brick_state.boss_intro_track.as_mut().unwrap().modify_state(|state_opt|{
    ...
    brick_state.boss_main_track.as_mut().unwrap().modify_state(|state_opt|{
        if state_opt.is_some(){
            ...
            EventKind::KeyDown { code } => {
                if let KeyCode::SPACE = code {
                    ...
                    if let GuiState::Menu = state {
                        ...
                        app_state.borrow_mut().boss_intro_track.as_mut().map(|track|{
